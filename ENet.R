...
#prediction test
set.seed(423)
test_ind=sample(nrow(dfTest), 500)
lasso_pred=predict(lassoM,newx=as.matrix(dfTest[test_ind, -1],ncol=30),
                   type='response',s='lambda.min')
lasso_pred_comp=data.frame(exp(dfTest[test_ind,1]),exp(lasso_pred))
...
row.names(df)[which(std_res_lasso>3)]

#mostly the model works ok. not so much during 08-09 period
